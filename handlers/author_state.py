...
async def author_search(message: types.Message, state: FSMContext):
    ...
    with con:
        ...
        if len(list_queries) == 0:
            query = cur.execute(
                "SELECT title, author, actors, url, description,alter_name, year FROM films_films WHERE author like ?",
                ('%' + message_l + '%',)).fetchall()
        elif len(list_queries) == 2:
            query = cur.execute(
                f"SELECT title, author, actors, url, description,alter_name, year FROM films_films WHERE author like ? and {list_queries[0]} like ?",
                ('%' + message_l + '%', list_queries[1])).fetchall()
        elif len(list_queries) == 4:
            query = cur.execute(
                f"SELECT title, author, actors, url, description,alter_name, year FROM films_films WHERE author like ? and {list_queries[0]} like ? and {list_queries[2]} like ?",
                ('%' + message_l + '%', list_queries[1], list_queries[3])).fetchall()
        elif len(list_queries) == 6:
            query = cur.execute(
                f"SELECT title, author, actors, url, description,alter_name, year FROM films_films WHERE author like ? and {list_queries[0]} like ? and {list_queries[2]} like ? and {list_queries[4]} like ?",
                ('%' + message_l + '%', list_queries[1], list_queries[3], list_queries[5])).fetchall()
        elif len(list_queries) == 8:
            query = cur.execute(
                f"SELECT title, author, actors, url, description,alter_name, year FROM films_films WHERE author like ? and {list_queries[0]} like ? and {list_queries[2]} like ? and {list_queries[4]} like ? and {list_queries[6]} like ?",
